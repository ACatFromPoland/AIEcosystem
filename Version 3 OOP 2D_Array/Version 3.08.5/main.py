        ...
        while True:
            ...
            try:
                if MapArray2D[xxx][yyy] == 0:
                    Entity_Dict.append(Rabbit(xxx, yyy, givenFood, creatureType, eyeSight, state))
                    Rabbit_Count = Rabbit_Count + 1
                    return
                else:
                    randomSpawn = random.randint(0, len(possibleLocations) - 1)
...
def GenerateEcoSystem(Num_of_Food=2, Num_of_Rabbits=1, Num_of_Foxes=0):
    ...
    for _ in range(Num_of_Rabbits):
        GenerateRabbit(False,False,5,5,0)
        GenerateRabbit(False,False,6,2,0)
        GenerateRabbit(False,False,5,7,0)
        GenerateRabbit(False,False,3,5,0)
        GenerateRabbit(False,False,8,1,0)
        GenerateRabbit(False,False,5,9,0)
